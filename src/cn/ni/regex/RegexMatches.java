...
//        System.out.println(sb.toString());
        ...
        try {
//            Pattern cn.ni.regex = Pattern.compile("(?<=<(\\w+)>).*(?=</\\1>)"); //会报错的
            Pattern regex = Pattern.compile("(?<=<(\\w)>)");
//            Pattern cn.ni.regex = Pattern.compile("(?<=<(\\w{1,10})>).*(?=</\\1>)"); //不会报错的  匹配exp后面的位置
//            Pattern cn.ni.regex = Pattern.compile("\\b\\w+(?=ing\\b)"); //不会报错的  匹配exp前面的位置
//            Pattern cn.ni.regex = Pattern.compile("(?!ing\\b)\\w+\\b"); //不会报错的  匹配后面跟的不是exp的位置
//            Pattern cn.ni.regex = Pattern.compile("\\b\\w+(?<!ing\\b)\\b"); //不会报错的  匹配前面跟的不是exp的位置
//            Pattern cn.ni.regex = Pattern.compile("2[0-4]\\d(/?#200-249)"); //java正则表达式应该没有注释一说

//            Matcher regexMatcher = cn.ni.regex.matcher("<asd>jessie</asd>");
//            Matcher regexMatcher = cn.ni.regex.matcher("ing");
